...
function MiniTrend({values,status}){const c=status==='critical'?palette.red:status==='warn'?palette.amber:palette.green;return <svg className="mini" viewBox="0 0 150 58" aria-hidden="true"><defs><linearGradient id={`g${c.slice(1)}`} x1="0" y1="0" x2="0" y2="1"><stop offset="0" stopColor={c} stopOpacity=".24"/><stop offset="1" stopColor={c} stopOpacity="0"/></linearGradient></defs><polygon points={`8,50 ${points(values,150,58,8)} 142,50`} fill={`url(#g${c.slice(1)})`}/><polyline points={points(values,150,58,8)} fill="none" stroke={c} strokeWidth="2.3"/><g fill={c}>{values.map((_,i)=>{const [x,y]=points(values,150,58,8).split(' ')[i].split(',');return <circle key={i} cx={x} cy={y} r="2.5"/>})}</g></svg>}
...
function Chart({spec}){const [title,type,data,meta,colorName]=spec,c=palette[colorName]||palette.green;if(type==='gauge'){const value=data;return <div className="chart"><div className="chart-title">{title}</div><div className="gauge" style={{'--value':`${value*1.8}deg`,'--gauge':c}}><div><strong>{String(value).replace('.',',')}%</strong><span>{meta}</span></div></div></div>}
 ...
 const series=type==='multi'?data:[data];return <div className="chart"><div className="chart-head"><span className="chart-title">{title}</span><span className="meta">{meta}</span></div><svg viewBox="0 0 300 145" className="chart-svg" role="img" aria-label={title}>{[30,65,100,135].map(y=><line key={y} x1="18" x2="286" y1={y} y2={y} stroke="#dce5f1" strokeWidth="1"/>)}{type==='bar'?data.map((v,i)=>{const max=Math.max(...data)*1.12;const h=105*v/max;return <rect key={i} x={22+i*260/data.length} y={132-h} width={Math.max(8,260/data.length-7)} height={h} rx="2" fill={c}/>}):series.map((s,idx)=><g key={idx}><polyline points={points(s,290,120,18)} transform="translate(0 11)" fill="none" stroke={[palette.green,palette.amber,palette.blue][idx]||c} strokeWidth="3"/><g fill={[palette.green,palette.amber,palette.blue][idx]||c}>{s.map((_,i)=>{const [x,y]=points(s,290,120,18).split(' ')[i].split(',');return <circle key={i} cx={x} cy={Number(y)+11} r="3"/>})}</g></g>)}</svg><div className="axis">{(data.length<=4?['Q2','Q3','Q4','Q1']:months).slice(0,Array.isArray(data[0])?12:data.length).map(x=><span key={x}>{x}</span>)}</div></div>}
...
function Sidebar({open,setOpen,current,setCurrent}){return <><button className="menu-btn" onClick={()=>setOpen(!open)} aria-label={open?'Cerrar menú':'Abrir menú'}>{open?<PanelLeftClose/>:<Menu/>}<span>Tableros</span></button>{open&&<button className="scrim" onClick={()=>setOpen(false)} aria-label="Cerrar menú"/>}<aside className={`sidebar ${open?'open':''}`}><div className="side-head"><div className="side-logo"><Zap/>SolarTech</div><button onClick={()=>setOpen(false)} aria-label="Cerrar"><X/></button></div><p>Tableros estratégicos</p><nav>{dashboards.map((d,i)=>{const Icon=d.icon;return <button key={d.id} className={current===i?'active':''} onClick={()=>{setCurrent(i);setOpen(false);window.scrollTo({top:0,behavior:'smooth'})}}><span className="nav-icon"><Icon/></span><span><small>0{i+1}</small>{d.short}</span></button>})}</nav><div className="side-foot"><Leaf/><span>Transición energética<br/><b>SolarTech 2030</b></span></div></aside></>}

function KPI({kpi}){const [code,label,value,goal,status,Icon,values]=kpi;return <article className={`kpi ${status}`}><div className="kpi-icon"><Icon/></div><div className="kpi-main"><h3><b>{code}</b> · {label}</h3><strong className="kpi-value">{value}</strong><div className="kpi-meta"><span>META: <b>{goal}</b></span><span className="state"><i>{status==='critical'?'!':status==='warn'?'!':'✓'}</i>{status==='critical'?'Crítico':status==='warn'?'Atención':'Cumple'}</span></div></div><MiniTrend values={values} status={status}/></article>}

function Filter({item,index}){const icons=[CalendarDays,MapPin,Route,Building2,Users,PackageCheck];const Icon=icons[index%icons.length];return <div className="filter"><span><Icon/>{item[0]}</span><div>{item[1]}<ChevronDown/></div></div>}

function Dashboard({data}){return <main className="dashboard"><section className="title-row"><div><h1>{data.title}</h1><p>{data.subtitle}</p></div><div className="filters">{data.filters.map((f,i)=><Filter item={f} index={i} key={f[0]}/>)}</div></section><section className={`kpi-grid k${data.kpis.length}`}>{data.kpis.map(k=><KPI kpi={k} key={k[0]}/>)}</section><section className={`sections s${data.sections.length}`}>{data.sections.map((section,i)=><article className="panel" key={section.title}><div className="panel-head"><span className="panel-icon">{React.createElement(data.icon)}</span><div><h2>{section.title}</h2><p>{section.subtitle}</p></div></div><div className={`charts c${section.charts.length}`}>{section.charts.map((chart,j)=><Chart spec={chart} key={j}/>)}</div><div className="insight"><span><Check/></span><p>{i===0?'Los indicadores mantienen una evolución consistente y alineada con los objetivos estratégicos.':'El seguimiento integrado permite anticipar desvíos y sostener la mejora continua.'}</p></div></article>)}</section><section className="value-chain"><div><Activity/><span><b>Resumen estratégico</b>Cadena de valor y estado consolidado</span></div>{data.kpis.slice(0,4).map(k=><div key={k[0]}><span className={`dot ${k[4]}`}/><span><b>{k[0]}</b>{k[2]}</span></div>)}</section></main>}

function App(){const [current,setCurrent]=useState(0),[open,setOpen]=useState(false);return <><Header/><Sidebar open={open} setOpen={setOpen} current={current} setCurrent={setCurrent}/><Dashboard data={dashboards[current]}/></>}
