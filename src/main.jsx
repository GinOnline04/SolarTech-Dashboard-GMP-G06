...
import './styles.css';
import cordobaMap from './assets/cordoba-map.png';
...
function ExpansionMetric({kpi,index}){const [code,label,value,goal,status,Icon]=kpi;return <article className={`exp-metric ${status}`} style={{animationDelay:`${index*.08}s`}}><div className="exp-metric-head"><span className="exp-metric-icon"><Icon/></span><div><h3><b>{code}</b> · {label}</h3><small>({code})</small></div></div><div className="exp-metric-value"><strong>{value}</strong><span>META <b>{goal}</b></span></div><div className="exp-status"><i/>{status==='warn'?'Estado: Atención':'Estado: Cumple'}</div></article>}

function TrafficLegend(){return <div className="traffic-legend"><span><i className="ok"/>Cumple</span><span><i className="warn"/>Atención</span><span><i className="critical"/>No conformidad</span></div>}

function TerritoryMap(){return <aside className="territory-map" aria-label="Red de nodos estratégicos en Córdoba"><div className="map-stage"><img src={cordobaMap} alt="Mapa de la provincia de Córdoba"/><span className="node-route"/><div className="map-node north"><i/><span>Nodo<br/><b>Norte</b></span></div><div className="map-node central"><i/><span>Almacén<br/><b>Central</b></span></div><div className="map-node south"><i/><span>Nodo<br/><b>Sur</b></span></div></div><p>Red de nodos estratégicos<br/>en la provincia de Córdoba</p></aside>}

function ExpansionDashboard({data,selections,onFilterChange}){const [openFilter,setOpenFilter]=useState(null);useEffect(()=>setOpenFilter(null),[data.id]);const operational=data.kpis.slice(0,3);const cav=data.kpis[3];return <main className="dashboard expansion-dashboard"><section className="expansion-title"><span className="expansion-title-icon"><Map/></span><div><h1>{data.title}</h1><p>{data.subtitle}</p></div></section><section className="expansion-filters">{data.filters.map((f,i)=><Filter item={f} index={i} value={selections[i]||f[1]} isOpen={openFilter===i} onToggle={()=>setOpenFilter(openFilter===i?null:i)} onSelect={value=>{setOpenFilter(null);onFilterChange(i,value)}} key={f[0]}/>)}</section><section className="expansion-layout"><TerritoryMap/><article className="expansion-block operational-block"><header><MapPin/><h2>1. Expansión operativa</h2></header><div className="exp-metrics">{operational.map((kpi,i)=><ExpansionMetric kpi={kpi} index={i} key={kpi[0]}/>)}</div><div className="expansion-chart-grid">{data.sections[0].charts.map((spec,i)=><article className="exp-chart-card" key={spec[0]}><Chart spec={spec}/><TrafficLegend/></article>)}</div></article><article className="expansion-block self-block"><header><Leaf/><h2>2. Autosuficiencia corporativa</h2></header><ExpansionMetric kpi={cav} index={3}/><article className="exp-chart-card cav-chart"><Chart spec={data.sections[1].charts[0]}/><TrafficLegend/></article></article></section></main>}
